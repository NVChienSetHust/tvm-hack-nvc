...
reg.register_strategy("nn.mcuconv2davg", strategy.conv2d_strategy)
reg.register_pattern("nn.mcuconv2davg", OpPattern.OUT_ELEMWISE_FUSABLE)
    
@reg.register_legalize("nn.mcuconv2davg", level=10)
def mcu_nn_conv2d_avg(attrs, inputs, types):
    new_inputs = [relay.cast(_, "int32") for _ in inputs]
    x, y, b, zx, zy, scale, order = new_inputs
    nx = x - zx 
    conv_res = relay.nn.conv2d(nx, y, attrs.strides, attrs.padding, attrs.dilation, attrs.groups)
    conv_res = relay.nn.bias_add(conv_res, b)
    scale = relay.reshape(scale, newshape=[1, -1, 1, 1])
